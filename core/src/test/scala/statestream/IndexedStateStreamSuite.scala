...
trait IndexedStateStreamSuite[F[_], G[_], H[_], I[_]] extends FunSuite with PropertyChecks {
  import IndexedStateStreamSuite._
  ...
  def mkStateStream[S, A](src: F[A]): IndexedStateStream[F, G, H, I, S, S, A]
  def mkStateStream[S: Monoid, A](src: F[(S, A)]): IndexedStateStream[F, G, H, I, S, S, A]
  ...
  def runConcat[S: Monoid, A, B](
    source: F[(S, A)]
  )(f: IndexedStateStream[F, G, H, I, S, S, A] => IndexedStateStream[F, G, H, I, S, S, B]): Seq[(S, B)] =
    runStream(f(mkStateStream[S, A](source)).stream).map(v => extract(v.runEmpty))
  ...
  test("can map over state") {
    forAll { (state: Int, value: String) =>
      val ret = runStream(mkStateStream[Int, String](F.pure(value)).map(_.length).stream)
  ...
  test("can mapAsync over state") {
    forAll { values: NonEmptyList[(Int, String)] =>
      val ret =
        runStream(
          mkStateStream[Int, String](F.fromSeq(values.toList)).mapAsync(parallelism)(v => H.pure(v.length)).stream
        )

      assert(ret.size === values.size && ret.map(_.runEmpty).map(extract).toList === values.map {
        case (s, v) => (s, v.length)
      }.toList)
    }
  }
  ...
  test("can flatMap state") {
    forAll { (state: Int, value: String) =>
      val ret =
        runStream(
          mkStateStream[Int, String](F.pure(value)).flatMapS(v => StateT(s => G.pure((v.length + s, v)))).stream
        )
  ...
  test("can flatMapAsync state") {
    forAll { (state: Int, value: String) =>
      val ret = runStream(
        mkStateStream[Int, String](F.pure(value))
          .flatMapAsync(parallelism)(v => StateT(s => H.pure(v.length + s -> v)))
          .stream
      )
  ...
  test("can modify state") {
    forAll { (state: Int, value: String) =>
      val ret =
        runStream(mkStateStream[Int, String](F.pure(value)).modify(_.toString).stream)
  ...
  test("can concatenate collection, putting state ") {
    forAll { (state: Int, value: String) =>
      val ret = runStream(mkStateStream[Int, String](F.pure(value)).transform(_.toString -> _.length).stream)
  ...
  test("converts state and data to a tuple") {
    forAll { (state: Int, value: String) =>
      val data = runStream(mkStateStream[Int, String](F.pure(state -> value)).toTuple)
  ...
  test("converts state and data to a tuple when provided initial state") {
    forAll { (state: Int, value: String) =>
      val data = runStream(mkStateStream[Int, String](F.pure(value)).toTuple(state))
  ...
  test("can send state and data by independent flows") {
    forAll { values: NonEmptyList[(Int, String)] =>
      val data = runStream(
        mkStateStream[Int, String](F.fromSeq(values.toList))
          .via(mkPipe(_.toString), mkPipe(_.length))
          .stream
      )

      assert(data.map(v => extract(v.runEmpty)) === values.map { case (s, d) => s.toString -> d.length }.toList)
    }
  }
  ...
  test("can send state by separate flows") {
    forAll { values: NonEmptyList[(Int, String)] =>
      ...
      assert(data.map(v => extract(v.runEmpty)) === values.map { case (s, d) => s.toString -> d }.toList)
    }
  }
  ...
  test("can send data by separate flow") {
    forAll { values: NonEmptyList[(Int, String)] =>
      ...
      assert(data.map(v => extract(v.runEmpty)) === values.map { case (s, d) => s -> d.length }.toList)
    }
  }
  ...
  test("can send state and data to separate sinks when provided with an initial state") {
    forAll { (initialState: Int, values: NonEmptyList[String]) =>
      val (state, data) = materialize(
        mkStateStream[Int, String](F.fromSeq(values.toList))
          .toSinks[H[Seq[Int]], H[Seq[String]]](initialState)(mkSeqSink, mkSeqSink)
      )

      assert(waitFor(state) === repeat(initialState)(values.size) && waitFor(data) === values.toList)
    }
  }
  ...
  test("can materialize state and data with separate sinks and initial state, combining the results") {
    forAll { (initialState: Int, values: NonEmptyList[String]) =>
      val (stateRes, valueRes) = waitFor(
        materialize(
          mkStateStream[Int, String](F.fromSeq(values.toList))
            .toSinks[H[Seq[Int]], H[Seq[String]], H[(Seq[Int], Seq[String])]](initialState)(
              mkSeqSink[Int],
              mkSeqSink[String]
            )((l, r) => l.flatMap(ll => r.map((ll, _))))
        )
      ...
      assert(stateRes === repeat(initialState)(values.size) && valueRes === values.toList)
    }
  }
}

object IndexedStateStreamSuite {
